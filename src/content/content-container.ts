...
function addContainerElement({
  ...
  if (canUseTopLayer()) {
    ...
    // Clicks outside an automatic popover dismiss it, even when they target
    // our manual popover. Keep our controls inside the page's open popover so
    // interacting with the dictionary or puck does not close the source text.
    // Ignore popovers outside the active modal/fullscreen subtree, where our
    // controls would be inert, and our own (manual) containers.
    const pagePopovers = parent.querySelectorAll<HTMLElement>(
      '[popover]:popover-open'
    );
    for (const popover of pagePopovers) {
      if (popover.popover !== 'manual') {
        parent = popover;
      }
    }
...
function onPopoverToggle(event: Event) {
  if (
    ...
    !containers.has(event.target.id)
  ) {
    updateContainers();
  }
}
