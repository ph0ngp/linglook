...
export function KanjiReadings(props: Props) {
  ...
  return (
    <>
      {props.r.py && props.r.py.length > 0 && (
        <div lang="zh" class="tp-text-[--reading-highlight] tp-text-base">
          {props.r.py.join(', ') || null}
        </div>
      )}
      {/* CY: TODOP don't know why but simply props.r.on?.length && will display "0" */}
      {props.r.on && props.r.on.length > 0 && (
        <div lang="vi" class="tp-text-[--reading-highlight] tp-text-base">
