    ...
    if (chosenWord) {
      ...
      result.kanji = {
        ...
        data: allCharsToShowStrokes.map((char) => {
          ...
          const radicalStr = charData[6];
          let radicalText = '';
          if (radicalStr && radicalStr !== '*') {
            const radical = radicalStr.split('|')[0];
            const gloss = radicalStr.split('|')[1];
            radicalText = `${radical} (${gloss})`;
          }
